  ...
  text = ''
    ...
    usage() {
      printf '%s\n' \
        ...
        "" \
        "Set JUMP_LOCAL_IMAGE_TRANSPORT=docker-daemon to load into Docker."
    }
    ...
    case "$action" in
      ...
      load)
        ...
        tag="''${1:-dev}"
        validate_tag "$tag"
        local_transport="''${JUMP_LOCAL_IMAGE_TRANSPORT:-containers-storage}"
        case "$local_transport" in
          containers-storage|docker-daemon) ;;
          *)
            echo "JUMP_LOCAL_IMAGE_TRANSPORT must be containers-storage or docker-daemon" >&2
            exit 2
            ;;
        esac
