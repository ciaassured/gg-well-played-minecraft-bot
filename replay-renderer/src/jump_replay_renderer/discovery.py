...
def discover_replays(source: Path) -> tuple[Path, list[DiscoveredReplay]]:
    ...
    replay_root = resolved / "replays" if (resolved / "replays").is_dir() else resolved
    files = sorted(
        path
        for path in replay_root.rglob("*.mcpr")
        if path.is_file() and "videos" not in path.relative_to(replay_root).parts
    )
    if not files:
